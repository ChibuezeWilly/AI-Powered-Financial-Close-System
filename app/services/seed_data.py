...
def _dates_in_month(
    year: int, month: int, rng: random.Random, count: int
) -> list[date]:
...
def seed_all(db: Session) -> dict:
    ...
    tx_counter, inv_counter, pay_counter, bank_counter, je_counter, disc_counter = (
        2001,
        2001,
        3001,
        4001,
        ...
        for i in range(tx_count):
            ...
            if is_discrepancy:
                ...
                if disc_type == "UNDOCUMENTED_DISCOUNT":
                    ...
                    recommendation = (
                        f"Contact customer for remaining balance of ${diff:.2f}."
                    )
                elif disc_type == "DUPLICATE_PAYMENT":
                    actual, diff, severity = (
                        round(total * 2, 2),
                        round(-total, 2),
                        "CRITICAL",
                    )
                    ...
                    recommendation = (
                        f"Park ${total:.2f} as unapplied cash and process refund."
                    )
                    ...
                    recommendation = (
                        f"Hold ${overage:.2f} as unapplied cash and contact customer."
                    )
                    ...
                    recommendation = (
                        "No financial adjustment needed. Record timing note."
                    )
                elif disc_type == "BANK_FEE":
                    fee = round(rng.uniform(15, 75), 2)
                    actual, diff = round(total - fee, 2), round(fee, 2)
                    root_cause = (
                        f"Bank processing fee of ${fee:.2f} deducted from payment."
                    )
                ...
                status = rng.choice(
                    ["DISCREPANCY_DETECTED", "INVESTIGATING", "AWAITING_HUMAN_APPROVAL"]
                )
                confidence = round(rng.uniform(0.72, 0.96), 2)
            else:
                actual, diff, disc_type, severity, status = (
                    total,
                    0.0,
                    None,
                    "NONE",
                ...
                root_cause, recommendation, confidence, account = (
                    None,
                    None,
                    None,
                    "Accounts Receivable",
            ...
            if actual > 0:
                db.merge(
                    Payment(
                        ...
                        currency=customer.get("currency", "USD"),
                        method=rng.choice(
                            ["wire_transfer", "ach", "check", "credit_card"]
                        ),
            ...
            # 💡 2. Explicitly seed the parent JournalEntry row right before the child rows evaluate
            db.merge(
                JournalEntry(
                    id=je_id,
                    description=f"Journal entry for invoice {inv_id}",
                    accounting_period=period,
            ...
            db.merge(
                FinancialTransaction(
                    id=tx_id,
                    period=period,
                    transaction_date=str(inv_date),
                    customer=customer["name"],
                    customer_id=customer["id"],
                    invoice_id=inv_id,
                    payment_id=pay_id if actual > 0 else None,
            ...
            if is_discrepancy:
                ...
                db.merge(
                    DiscrepancyGroundTruth(
                        ...
                        required_policy=(
                            "FIN-042" if "DISCOUNT" in (disc_type or "") else "FIN-001"
                        ),
                        expected_action=(
                            "approve_adjustment"
                            if "DISCOUNT" in (disc_type or "")
                            else "investigate"
                        ),
                    )
                )
